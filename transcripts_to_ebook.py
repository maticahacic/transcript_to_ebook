...
DEVELOPER_KEY = "AIzaSyDc15jnnRmFk7P2cpwUKf6s4hTL0jY3usA"
...
def get_video_details(type_of_detail):
    ...
    list_videos_byid = youtube.videos().list(id=video_id, part="id, snippet, contentDetails").execute()
    # extracting the results from search response
    results = list_videos_byid.get("items", [])
    print(results)
    title = results[0]["snippet"]["title"]
    description = results[0]["snippet"]["description"]
    imgurl = results[0]["snippet"]["thumbnails"]["maxres"]["url"]
    author = results[0]["snippet"]["channelTitle"]
    if type_of_detail == "title":
        return title
    elif type_of_detail == "description":
        return description
    elif type_of_detail == "imgurl":
        return imgurl
...
def show_autogenerated_transcripts(data):
    ...
    video_id = get_video_id(dpg.get_value("youtubethingy"))
    transcript_list_list = get_list_of_available_languages(video_id, data)
    transcript_list_manual = []
    transcript_list_generated = []
    transcript_list = YouTubeTranscriptApi.list_transcripts(video_id)
...
def do_the_youtube_thingy(sender):
    ...
    if validate_youtube_video_id_url(video_id):
        ...
        # draw transcript
        time_stamps_chkbox = dpg.get_value("timestamps_chkbx")
